...
| Application assist profile-sync adapter | `browser/application-assist/profile-sync.js` | tracked product runtime adapter | update validation/projection/cockpit active-tab sync seam (acceptOnlyKnownProfileKeys, capped 64 KiB, legacy schemas) without adding progression/password/final-submit; no broad host permissions, no network requests | `node tests/test_profile_sync.mjs` and `python tests/test_application_assist_session_contract.py` |
| Application assist docs | `docs/APPLICATION_ASSIST_SESSION.md` | tracked operator docs | update load/control-loop and profile-bootstrap/path-hygiene instructions without real PII; document one-action cockpit sync and `Outputs/application-assist/` | `python tests/test_application_assist_session_contract.py` |
...
| Application resume artifact durability contract | `contracts/application-resume-artifact-sync.v1.json` | tracked versioned sync contract | require provider-backed Drive identities and tracker parity before DURABLE_READY; never store private resume content | `python scripts/validate_application_resume_artifact_sync.py` |
| Application resume artifact durability validator | `scripts/validate_application_resume_artifact_sync.py` | tracked sync-contract validator | enforce local-only negative case, provider read-back, and tracker/Drive identity parity | `python scripts/validate_application_resume_artifact_sync.py` |
| Application resume artifact durability tests | `tests/test_application_resume_artifact_sync.py` | tracked semantic regression tests | preserve positive durable control plus local-only and stale-link negative cases | `python tests/test_application_resume_artifact_sync.py` |
...
| Application assist recovery export | user | portable profile JSON (`escapehatch-application-assist-profile/v2` plus legacy `v1`/`escapehatch-application-autofill-profile/v1`, 64 KiB cap, schema-checked, field-whitelisted, never executed) with deterministic prefix `escapehatch-application-assist-profile`; legacy phone defaults to `unconfirmed` | explicit browser download to user-selected default (usually `Downloads`); not a build artifact under `Outputs/application-assist/`; restore via Import or one-action cockpit sync |
| Generated application-assist artifact root (local, not tracked) | user/build | produced files under `Outputs/application-assist/` from local `browser/application-assist` build | local generated output; single canonical generated-output root; do not commit real profile values; Git-ignored except for `.gitkeep` if present |
