...
| Generated application-assist artifact root (local, not tracked) | user/build | produced files under `Outputs/application-assist/` from local `browser/application-assist` build | local generated output; do not commit real profile values; Git-ignored except for `.gitkeep` if present |
