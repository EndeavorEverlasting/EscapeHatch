...
for (const [descriptor, expected] of cases) assert.equal(core.classifyField(descriptor), expected, JSON.stringify(descriptor));
...
const profile = {
  name_prefix: "Mr.", first_name: "Alex", last_name: "Example", preferred_name: "Alex",
  email: "alex@example.invalid", phone: "+1 555 010 0142", linkedin_url: "https://www.linkedin.com/in/example-candidate",
  street_address: "123 Example Avenue", city: "Example City", region: "NY", postal_code: "00000", country: "US",
...
const fields = [
  ...
  { label: "First Name", value: "" }, { label: "Last Name", value: "Already Present" }, { label: "Email", value: "" },
  { label: "Phone Number", value: "" }, { label: "LinkedIn Profile", value: "" }, { label: "Home Street Address", value: "" },
  { label: "City / Municipality", value: "" }, { label: "Zip Code", value: "" }, { label: "Questionnaire response", value: "" },
  { label: "Resume", type: "file", value: "" }, { label: "Email", type: "email", hidden: true, value: "" }
];
const assignments = core.planAssignments(fields, profile);
assert.deepEqual(assignments.map((item) => item.key), ["name_prefix","first_name","email","phone","linkedin_url","street_address","city","postal_code"]);
...
class FakeInput extends EventTarget {
  constructor({ label, autocomplete = "", value = "", type = "text", name = "", id = "", hidden = false }) {
    super(); this.tagName = "INPUT"; this.type = type; this.name = name; this.id = id; this.placeholder = "";
    this.disabled = false; this.readOnly = false; this.hidden = hidden; this.isConnected = true; this.labels = [{ textContent: label }];
    this._attrs = { autocomplete }; this._value = value;
  }
...
const events = [];
first.addEventListener("input", () => { events.push("first:input"); email.value = "framework@already.invalid"; });
first.addEventListener("change", () => events.push("first:change"));
email.addEventListener("input", () => events.push("email:input")); email.addEventListener("change", () => events.push("email:change"));
const filled = core.fillDocument({ querySelectorAll: () => [first, email] }, { first_name: "Alex", email: "alex@example.invalid" });
...
const hiddenEmail = new FakeInput({ label: "Email", hidden: true });
const hiddenResult = core.fillDocument({ querySelectorAll: () => [hiddenEmail] }, { email: "alex@example.invalid" });
assert.equal(hiddenEmail.value, "", "visibly hidden controls must not receive profile data");
assert.equal(hiddenResult.filled, 0);

const detached = new FakeInput({ label: "Last Name" }); detached.isConnected = false;
const detachedResult = core.fillDocument({ querySelectorAll: () => [detached] }, { last_name: "Example" });
assert.equal(detached.value, ""); assert.equal(detachedResult.filled, 0);
